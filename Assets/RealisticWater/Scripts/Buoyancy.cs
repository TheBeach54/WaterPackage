...
public class Buoyancy : MonoBehaviour
{
    ...
    private List<Vector3> waterContact = new List<Vector3>();
    ...
    private Vector4 gAmplitude, gWavelength, gPhase, gDirection, gOtDirection;
    ...
    private void Start()
    {
        ...
        if (waterMaterial != null)
        {
            ...
            gWavesPos = Shader.GetGlobalVectorArray("_GWavesPos");
        }
    ...
    private float GetWaterLevel(float x, float z)
    {
        ...
        return GetGrestnerWavePos(new Vector3(x,0.0f, z));
    }

    public bool GetInWater(float bias = 0.0f)
    {
        return inWater;
    }
    ...
    private void FixedUpdate()
    {
        ...
        foreach (var point in voxels)
        {
            ...
            if (wp.y - voxelHalfHeight < waterLevel)
            {
                ...
                waterContact.Add(new Vector3(wp.x, waterLevel, wp.z));
        ...
        if (inWater)
            ...
            rb.drag -= Time.fixedDeltaTime*10.0f;
    ...
    private void OnDrawGizmos()
    {
        ...
        foreach (var w in waterContact)
        {
            Gizmos.DrawSphere(w, 0.5f);
        }
    ...
    Vector2 GetCircularDirection(Vector3 wPos, Vector2 gPos)
    {

        return (gPos - new Vector2(wPos.z,wPos.x).normalized);

    }
    public float GetGrestnerWavePos(Vector3 worldPos)
    {


        float time = Time.fixedTime;
            worldPos = new Vector3(worldPos.z, worldPos.x, worldPos.y);
        ...
        for (int i = 0; i < count; i++)
            ...
            if( gWaves[i].w > 450.0f)
            {
                D = GetCircularDirection(worldPos, new Vector2(gWavesPos[i].x, gWavesPos[i].y));
            }
            else
            {

            D = new Vector2(Mathf.Cos(gWaves[i].w), Mathf.Sin(gWaves[i].w));
            }




            Zp += A * Mathf.Sin(Vector2.Dot(w * D, new Vector2(worldPos.x, worldPos.y)) + p * time);
            }
    ...
    public Vector3 GetAverageGrestnerNormal(Vector3 worldPos)
    {
        ...
        foreach (var point in voxels)
        {
            var wp = transform.TransformPoint(point);
                ...
                waterNormal += GetGrestnerNormalSurface(wp);

            
        }

        return waterNormal / (float)count;
    }

    public Vector3 GetGrestnerNormalSurface(Vector3 worldPos)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            Q = Mathf.Clamp01(gSteepness) / (w * A);

            WA = w * A;
            S = Mathf.Sin(w * Vector2.Dot(D, wP) + p * time);
            C = Mathf.Cos(w * Vector2.Dot(D, wP) + p * time);
        ...
        return new Vector3(Ny, Nz, Nx).normalized;
    }
