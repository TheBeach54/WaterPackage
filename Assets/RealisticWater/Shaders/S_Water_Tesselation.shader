...
Shader "Beach/Water_Tesselation" {
	Properties{
	...
	[Toggle(_DEBUG_FRESNEL)]_DebugFresnel("DEBUG : Visualize Fresnel (0=Mirror,1=Transparent)", Float) = 0
		...
		_SunDiscScale("Sun Scale", Range(0,4)) = 0.005
	...
	[Toggle(_USE_GRESTNER_WAVE)]_UseGrestnerWaves("Use Grestner Procedural Waves (variant)", Float) = 0
		...
		_GSteepness("Wave Steepness", Float) = 0
	...
	_FoamPower("Foam Power", Float) = 1
		...
		_FoamMaskSpeed("_FoamMaskSpeed", Vector) = (0, 0, 0, 1)
		_FoamColor("_FoamColor", Color) = (1, 1, 1, 0)
		_FoamMap("Foam Mask", 2D) = "white" {}
	...
	//[HideInInspector]_WaveTime("_WaveTime", Float) = 0
...
#include "Water.cginc"
	...
	float _TessDistMin, _TessDistMax, _TessDistPow;
	float _MaxTess, _MinTess;


	hullIn vert(appdata v)
	{
	hullIn o;

	o.vertex = mul(unity_ObjectToWorld, v.vertex);

	float distLerper = InverseLerp(_TessDistMin, _TessDistMax, distance(_WorldSpaceCameraPos, o.vertex.xyz));
	...
	HullConstantOutput hullConstant(InputPatch<hullIn, 3> i)
	{
	HullConstantOutput o;
	o.TessFactor[0] = 0.5f*(i[1].tessFac + i[2].tessFac);
	o.TessFactor[1] = 0.5f*(i[2].tessFac + i[0].tessFac);
	o.TessFactor[2] = 0.5f*(i[0].tessFac + i[1].tessFac);
	...
	domainIn hull(InputPatch < hullIn, 3> i, uint uCPID : SV_OutputControlPointID)
	{
	domainIn o;
	...
	[domain("tri")]
	v2f domain(HullConstantOutput constantData, const OutputPatch<domainIn, 3> i, float3 bary : SV_DomainLocation)
	{
	v2f o;
	...
	//float2 waveDir = normalize(worldPos.xyz - float3(_GDirection.x,0.0f,_GDirection.y)).xz;
	//
	//float2 waveDir2 = normalize(worldPos.xyz - float3(_GDirection.z, 0.0f, _GDirection.w)).xz;

	float3 tempNormal = 0;
	GerstnerWave(worldPos.xyz, tempNormal, _WaveTime);



	#endif
	//worldNormal = GerstnerNormal(worldPos, _Time.x);
	...
	float disTest = step(distance(_WorldSpaceCameraPos, position), _TessDistMax);

	float simTest = step(simUV.x, 0.9999f);
	simTest *= 1 - step(simUV.x, 0.00001f);
	simTest *= step(simUV.y, 0.9999f);
	simTest *= 1 - step(simUV.y, 0.00001f);

	_Displacement *= simTest * disTest;

	DynamicDisplace(_DynamicHeight, simUV, float3(0, 1, 0), _Displacement, worldPos);
...
#include "Water.cginc"
	...
	sampler2D_float _OrthoCollisionsTex;

	sampler2D _FoamMap;
	float _FoamPower, _FoamMultiplier;
	float4 _FoamMaskSpeed;
	float4 _FoamColor;
	float _FoamMaskScale, _FoamScale;
	...
	hullIn vert(appdata v)
	{
		...
		o.binormal = normalize(cross(o.normal_tessFac.xyz, o.tangent) * tangentSign);
	...
	v2f domain(HullConstantOutput constantData, const OutputPatch<domainIn, 3> i, float3 bary : SV_DomainLocation)
	{
		...
		float2 texcoord = bary.x * i[0].texcoord + bary.y * i[1].texcoord + bary.z * i[2].texcoord;
		float4 color = bary.x * i[0].color + bary.y * i[1].color + bary.z * i[2].color;
		...
		worldNormal = tempNormal;

		tangent = normalize(cross(worldNormal, binormal));
		binormal = normalize(cross(tangent, worldNormal));
		...
		o.pack0.xy = TRANSFORM_TEX(texcoord, _BumpMap);
		...
		o.eyeD.x = -mul(UNITY_MATRIX_V, worldPos).z;
...
//	return float4(worldNor.xzy*0.5+0.5, 1.0f);
	float time = _Time.x;
	...
	float2 simUV = float2(InverseLerp(_SimulationPosition.x - 0.5f * _SimulationPosition.w, _SimulationPosition.x + 0.5f * _SimulationPosition.w, worldPos.x), InverseLerp(_SimulationPosition.z - 0.5f * _SimulationPosition.w, _SimulationPosition.z + 0.5f * _SimulationPosition.w, worldPos.z));
		...
		float simTest = i.eyeD.y;
			

		dynamicNM.xy *= _NormalPower;
		dynamicNM = normalize(dynamicNM);

		float3 tempFinalNM = finalNM * 0.5 + 0.5;
		float3 tempFresnelNM = fresnelNM * 0.5 + 0.5;
		dynamicNM = dynamicNM * 0.5 + 0.5;

		float3 t = dynamicNM*float3(2, 2, 2) + float3(-1, -1, 0);
		float3 u = tempFinalNM*float3(-2, -2, 2) + float3(1, 1, -1);
		float3 r = t*dot(t, u) / t.z - u;
		//return r*0.5 + 0.5;

		float3 uu = tempFresnelNM*float3(-2, -2, 2) + float3(1, 1, -1);
		float3 rr = t*dot(t, uu) / t.z - uu;


		finalNM = lerp(finalNM,normalize(r),simTest);
		//return float4(dynamicNM * 0.5 + 0.5, 1);
		fresnelNM = lerp(fresnelNM,normalize(rr), simTest);

		//float dynamicCollision = tex2D(_OrthoCollisionsTex, simUV);
		//
		//return float4(dynamicCollision.xxx, 1.0f);
	...
	float selfReflectTest = step(reflDir.y,0.1);

//	return float4(selfReflectTest.xxx, 1.0f);

	if (reflDir.y < 0)
		reflDir = reflect(reflDir, float3(0, 1, 0));// worldNormal;

	//reflDir.y = lerp(0.4, 1, saturate(reflDir.y));// lerp(reflDir.y, 1, selfReflectTest);
	...
	reflection.xyz = lerp(reflection.xyz, lerp(reflection.xyz, grabOpaque, 0.2f), selfReflectTest);

	//if (selfReflectTest > 0.5f)
	//spec = min(spec,0.25f);
#endif
...
#if _USE_DYNAMIC_WAVES || _USE_GRESTNER_WAVE
	...
	foamMask += tex2D(_DynamicHeight, simUV).w * simTest;


#endif
#if _USE_DYNAMIC_WAVES || _USE_GRESTNER_WAVE

	foamTex = tex2D(_FoamMap, (worldPos.xz*_FoamScale + time * _FoamMaskSpeed.zw)).y;

	finalFoam = foamTex.y * _FoamColor;

	spec *= max(1 - foamMask,0.2f);
	//foamMask = foamMask;
	grabFinal.xyz = lerp(grabFinal.xyz, finalFoam + shadingFinal * sunAtten, foamMask);
#endif
	...
	//float3 ttt = normalize(worldPos - float3(_GWavesPos[0].x, worldPos.y, _GWavesPos[0].y));
	//return float4(ttt, 1.0f);
	...
	Pass{
	Tags{ "Queue" = "Transparent" "ForceNoShadowCasting" = "True"  "LightMode" = "ForwardBase" }
	Cull Front
	ZWrite Off
	CGPROGRAM
...
#include "Water.cginc"
			...
			sampler2D_float _DynamicHeight;
	float _ShadowDisp;
	float _RefractionBias;
	float _SunDiscScale, _SunDiscPow;
	...
	v2f domain(HullConstantOutput constantData, const OutputPatch<domainIn, 3> i, float3 bary : SV_DomainLocation)
	{
		...
		//
		//float2 waveDir2 = normalize(worldPos.xyz - float3(_GDirection.z, 0.0f, _GDirection.w)).xz;
		float2 waveDir = normalize(float2(_GDirection.x, _GDirection2.x));
		float2 waveDir2 = normalize(float2(_GDirection.y, _GDirection2.y));

		float2 waveDir3 = normalize(float2(_GDirection.z, _GDirection2.z));
		float2 waveDir4 = normalize(float2(_GDirection.w, _GDirection2.w));
		...
		o.eyeD.zw = mul((float3x3)UNITY_MATRIX_V, worldNormal).xy;
	...
	float2 grabBumpUV = i.pack0.xy *_GrabPropScale;
	...
	float2 grabOffset = (grabDoubleBump + i.eyeD.zw * 0.1f) * _BumpGrabAmt * _UnderGrabAmt ;
	//i.uvgrab.xy = grabOffset * i.uvgrab.z + i.uvgrab.xy;

	float2 refrUV = uv + (grabOffset ) / i.screenUV.w;
	float2 refrBiasUV = uv + (grabOffset) / i.screenUV.w;
